...
export default function CountryNGOs({
  country,
  topicsFilter,
  headingOverride,
}: {
  ...
  const ngos = topicsFilter
    ? country.ngos.filter((n) =>
        Array.isArray(n.topics) && n.topics.some((t) => topicsFilter.includes(t)),
      )
    : country.ngos;
  ...
  return (
    ...
    >
      ...
      >
        {headingOverride ?? `Organizations working in ${country.name}`}
      </h2>
      ...
      {ngos.length === 0 ? (
        <p className="text-sm text-slate-600 italic">
          No listed organizations match this topic yet. Seablings is growing —
          check back as the community expands.
        </p>
      ) : (
